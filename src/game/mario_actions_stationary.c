...
s32 check_common_idle_cancels(struct MarioState *m) {
	...
	// NOTE: this could be re-used related for the flashlight, may not be needed tho
...
s32 act_idle(struct MarioState *m) {
	...
	// i may want to do something with this later
    /*if (!(m->actionArg & 1) && m->health <= 30) {
        return set_mario_action(m, ACT_PANTING, 0);
    }*/
	
	// COMPLETED: check these for checks no longer wanted
    if (check_common_idle_cancels(m)) {
        return TRUE;
    }
	
	// NOTE: disabled this, probably should use it for the "look around"
...
s32 act_poltergust_sucking(struct MarioState *m) {
...
s32 check_common_landing_cancels(struct MarioState *m, UNUSED u32 action) {
    ...
    if (m->input & (INPUT_NONZERO_ANALOG | INPUT_OFF_FLOOR)) {
        return check_common_action_exits(m);
    }
...
s32 mario_execute_stationary_action(struct MarioState *m) {
    ...
    switch (m->action) {
		...
		case ACT_POLTERGUST_SUCKING:      cancel = act_poltergust_sucking(m);               break;
